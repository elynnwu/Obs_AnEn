...
for na in range(3):
    ...
    #    print 'Cloudy day: ', cloudy_test
        ...
        top5 = np.argsort(d2['d2_total'][:,0])[0:numAnalogs[na]]
    ...
    #        print 'The 5 most similar days: ', clear[clear_vars[0]].index[top5]
        ...
        plt.savefig('/mnt/lab_48tb1/database/Sc_group/AnEn/Results/zi_x2_dthetaL_x2_oceanLCC_x2_others_x1_'+str(numAnalogs[na])+'analogs/'+test_data_4am.index[i_test].strftime('%Y%m%d')+'.png',dpi=200,bbox_inches='tight')
        plt.close()
        ...
        np.savetxt('/mnt/lab_48tb1/database/Sc_group/AnEn/Results/zi_x2_dthetaL_x2_oceanLCC_x2_others_x1_'+str(numAnalogs[na])+'analogs/ts/'+test_data_4am.index[i_test].strftime('%Y%m%d')+'_GHI.csv',output,delimiter=',',fmt='%s')
    log_file.close()